...
export async function getAllDataFromFile(email) {
    ...
    const filePath = path.join(process.cwd(), 'data', 'AuthorizeData', '16thApril2026.json');
...
export async function createAndModifyUserInNeon(impInfo, email,first_name,last_name) {
    let settingsRecord = {
        type: 'subscription',
        status: impInfo?.nextPaymentDate == "N/A" ? "Inactive" : impInfo?.status,
        ...
        woocommerceAuthorizeImport: true,
