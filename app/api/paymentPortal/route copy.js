...
export async function POST(request) {
    ...
    */
    ...
    if (reason == "createSubscription") {
        ...
        //-- (flag) add AuthorizeNextImport - customer from authorize has merchantid than AuthNextImport is true, else its false
        ...
        let authorizationTransactionIs = await createAuthorizeTransaction()
        console.log("authorizationTransactionIs:", authorizationTransactionIs)

        if (authorizationTransactionIs.status) {
            let authorizeCustomerIs = await createCustomerInAuthorize(authorizationTransactionIs) //takes transactionId from authorizationTransactionIs
            console.log("authorizeCustomerIs:", authorizeCustomerIs)
            console.log("status is:", authorizeCustomerIs.status)
            if (authorizeCustomerIs.status) {
                ...
                if (subscriptionForCustomer.status) {
                    ...
                    let userInNeon = await createUserInDB(subscriptionForCustomer, authorizeCustomerIs)
                    console.log("userInNeon:", userInNeon)
                    return NextResponse.json({
    ...
    async function createSubscriptionInAuthorize(authorizeCustomerIs) {
        ...
        let termLength = incomingData.term == "quarterly" ? "3" : "12";
        ...
        let subscriptionName = "GymFit TV - $" + incomingData?.amount + " / " + incomingData?.term
        arbSubscription.setName(subscriptionName);
        arbSubscription.setPaymentSchedule(paymentScheduleType);
        arbSubscription.setAmount(incomingData.amount);
    ...
    async function createUserInDB(subscriptionForCustomer, authorizeCustomerIs) {
        const config = {
            headers: {
        ...
        let data = {
            ...
            next_payment_date_gmt: nextPaymentDate,
            ...
            authorizeNextImport: false,//alawys false in this route since new customer is created in authorize so that customer never has merchant id
            authorizeCustomerId: authorizeCustomerIs?.data?.customerProfileId
        }
        let result = null
        try {
