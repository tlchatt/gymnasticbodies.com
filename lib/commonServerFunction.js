...
export async function createSubscriptionInAuthorizeWithCustomerProfile(authorizeCustomerIs, impInfo, authorizationTransactionIs) {
    console.log("inside createSubscriptionInAuthorizeWithCustomerProfile")
    console.log("impInfo inside createSubscriptionInAuthorize:", impInfo)
    // console.log("inside createSubscriptionInAuthorize", authorizeCustomerIs?.data?.customerProfileId)

    console.log("authorizeCustomerIs:", authorizeCustomerIs)
    let paymentProfileId = authorizeCustomerIs?.data?.customerPaymentProfileIdList?.customerPaymentProfileId ?? "N/A"
    // let shippingId = authorizeCustomerIs?.data?.customerShippingAddressIdList ? authorizeCustomerIs?.data?.customerShippingAddressIdList[0] : "N/A"
    let customerProfileId = authorizeCustomerIs?.data?.customerProfileId ? authorizeCustomerIs?.data?.customerProfileId : "N/A"
    let startDate
...
export async function paymentController(formData) {
    ...
    */
    ...
    //define variables
    let customerSubscriptionStatus, authorizationTransactionIs, authorizeCustomerIs, subscriptionForCustomer, userInNeon, impInfo, customerData, customerSubscription, customerTransaction, customerProfile
    ...
    if (customerData) {//customer already present
        ...
        if (authorizationTransactionIs.status) {
            ...
            if (authorizeCustomerIs.status) {
                //get flag and subscription info for latercustomerData from allAuthorizeData:
                impInfo = await getFlagAndSubscriptionInfo(customerData, authorizeCustomerIs, incomingData)
                ...
                if (subscriptionForCustomer.status) {
                    ...
                    console.log("JSON.stringify", JSON.stringify({
                        ...
                        data: {
                            ...
                            impInfo: impInfo,
                            firstName: impInfo?.firstName ?? "N/A",
                            lastName: impInfo?.lastName ?? "N/A"
                        }
                    }))
                    return {
                        message: 'Transaction Successful, Customer Created, Subscription Created',
...
export async function updateCustomerPaymentProfile(paymentProfile, firstName, lastName, authorizeProfile) {
    ...
    updateRequest.setCustomerProfileId(authorizeProfile?.result?.profile?.customerProfileId);
