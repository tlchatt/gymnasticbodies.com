...
export async function createSubscriptionInAuthorizeWithCustomerProfile(authorizeCustomerIs, impInfo, authorizationTransactionIs) {
    console.log("authorizeCustomerIs inside createSubscriptionInAuthorize:", impInfo)
    // console.log("inside createSubscriptionInAuthorize", authorizeCustomerIs?.data?.customerProfileId)

    let paymentProfileId = authorizeCustomerIs?.data?.customerPaymentProfileIdList ? authorizeCustomerIs?.data?.customerPaymentProfileIdList[0]?.customerPaymentProfileId : "N/A"
    // let shippingId = authorizeCustomerIs?.data?.customerShippingAddressIdList ? authorizeCustomerIs?.data?.customerShippingAddressIdList[0] : "N/A"
    let customerProfileId = authorizeCustomerIs?.data?.customerProfileId ? authorizeCustomerIs?.data?.customerProfileId : "N/A"
    let startDate
    console.log("authorizeCustomerIs:", authorizeCustomerIs?.data?.customerPaymentProfileIdList[0])
    console.log("paymentProfileId:", paymentProfileId)
    // console.log("shippingId:", shippingId)
    console.log("customerProfileId:", customerProfileId)
    ...
    console.log("impInfo.recentTransactionDate:", impInfo.recentTransactionDate)
    console.log("impInfo.recentTransactionDate instanceof Date:", impInfo.recentTransactionDate instanceof Date)
    ...
    if (impInfo.recentTransactionDate) {
        ...
        //show error no recentTransactionDate provided
    }
    // else {
    //     startDate = new Date().toISOString().split('T')[0]; //todays date
    // }
    ...
    // var billTo = new ApiContracts.CustomerAddressType();
    // billTo.setLastName(impInfo.lastName);
...
export async function paymentFunctionality(formData) {
...
export async function updateCustomerPaymentProfile(paymentProfile,firstName, lastName) {
    console.log("paymentProfile:", paymentProfile?.data?.paymentProfile?.billTo)


    let merchantAuth = await authorizePaymentAuthentication()

    var creditCardForUpdate = new ApiContracts.CreditCardType();
    creditCardForUpdate.setCardNumber(paymentProfile?.data?.paymentProfile?.payment?.creditCard?.cardNumber);
    creditCardForUpdate.setExpirationDate(paymentProfile?.data?.paymentProfile?.payment?.creditCard?.expirationDate);
	...
	customerAddressType.setLastName(lastName ?? "N/A");
	customerAddressType.setZip(paymentProfile?.data?.paymentProfile?.billTo?.zip);
	customerAddressType.setCountry(paymentProfile?.data?.paymentProfile?.billTo?.country);
	
    var customerForUpdate = new ApiContracts.CustomerPaymentProfileExType();
    customerForUpdate.setPayment(paymentType);
    customerForUpdate.setCustomerPaymentProfileId(paymentProfile?.data?.paymentProfile?.customerPaymentProfileId);
    ...
    updateRequest.setCustomerProfileId(paymentProfile?.data?.paymentProfile?.customerProfileId);	
    ...
    var ctrl = new ApiControllers.GetCustomerProfileController(updateRequest.getJSON());
    ctrl.setEnvironment(SDKConstants.endpoint.production);
