...
export async function getAccountInformation(json) {
    let userEmail, customerData, usersettingInfo
    ...
    usersettingInfo = await queryUserSetting(json?.userId, json?.type)//get User Setting from neon db
    ...
    if (!customerId || customerId == "N/A") {
        customerData = await getAllDataFromFile(userEmail)//getting authorizeCustomerData from file
...
export async function getCorrectNameFormat(name) {
    let fullName = (name || "").trim().split(" ").filter(Boolean);//trims whitespace from beginning and end of the string. 
