    ...
    return (
        {
            ...
            subscriptionName: cleanPlanName(data?.productName) ?? "N/A",
...
export async function getAccountInformation(json) {
    ...
    const dbImpInfo = {
        ...
        country: data?.country ?? 'N/A',
        phoneNumber: data?.phone ?? 'N/A',
        // Status is derived from the expiration date (renewaldate), NOT the raw
        // user_setting.status string — see lib/subscription + status-from-expiration-audit.md.
        // trialEndDate supplies real in-trial evidence; the bare trial flag is never
        // cleared after signup, so it alone must not label a member 'Trial'.
        status: subscriptionStatusLabel({ renewaldate: data?.renewaldate, trial: usersettingInfo.trial, trialEndDate: usersettingInfo.trialEndDate }),
        isActive: isSubscriptionActive({ renewaldate: data?.renewaldate }),
        price: data?.price ?? 'N/A',
        ...
        subscriptionName: cleanPlanName(data?.productName) ?? 'GymFit Membership',
    ...
    if (usersettingInfo.stripeSubscriptionId) {
        try {
            ...
            impInfo = {
                ...
                // Live Stripe status is the correct basis in this branch; keep isActive in sync.
                isActive: isSubscriptionActive({ stripeStatus }),
    ...
    // Only enrich from Authorize.net when the user actually has a stored Auth.net customer
    // id (~19 legacy ARB subscribers). The old email-based getAllDataFromFile fallback
    // returned spurious customer ids for gateway-less users; Auth.net then returned an
    // "Error" response, and the `if (authorizeData?.result)` branch overwrote the good DB
    // profile (name/email) with blanks — the root cause of the blank My Account for migrated
    // users, plus a slow Auth.net API call on every account-page load.
    const resolvedAuthorizeId = (authorizeCustomerId && authorizeCustomerId !== 'N/A')
        ? authorizeCustomerId
        : null
    ...
    if (resolvedAuthorizeId) {
        try {
            ...
            if (authorizeData?.result) {
                ...
                // getFlagAndSubscriptionInfo derives `status` date-based (nextPaymentDate
                // vs today, canceled ARB => 'Inactive') but returns no isActive flag — and
                // the component's `impInfo?.isActive ?? true` default would treat every
                // expired Auth.net user as active (hiding the Renew button). Compute the
                // real flag here: future payment date, OR'd with the date-derived status
                // for legacy rows whose next payment date couldn't be computed.
                impInfo = {
                    ...impInfo,
                    isActive: isSubscriptionActive({ renewaldate: impInfo?.nextPaymentDate })
                ...
                return {
                    ...
                    migrationStatus: userInfo?.migrationType ?? null,
                    impInfo,
                }
            }
        } catch (e) {
