...
import { paymentFunctionality } from './commonServerFunction';
import { getUserWithEmail, queryUserSetting } from './userSettings';
...
export async function getFlagAndSubscriptionInfo(customerData, authorizeCustomerIs, incomingData) {
    ...
    console.log("retuned data from getFlagAndSubscriptionInfo", {
        ...
        firstName: incomingData?.firstName ? incomingData?.firstName : paymentProfile?.billTo?.firstName ?? "N/A",
        lastName: incomingData?.lastName ? incomingData?.lastName : paymentProfile?.billTo?.lastName ?? "N/A",
...
export async function paymentFormUpdate(response, amount, term, trial, formRef) {
    //create variables
    ...
    let payment = await paymentFunctionality(formData)
