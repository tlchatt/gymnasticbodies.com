...
const handlers = {
  ...
  async reply(email, response) {
    ...
    // The email is SENT by this point — a failure to record it must NOT mark the reply as failed
    // (that would misreport a real send and invite a duplicate on retry). Record best-effort.
    let recorded = false;
    if (t) {
      try {
        ...
        recorded = true;
      } catch (e) { recorded = `sent, but not recorded: ${e.message}`; }
    }
    return { ticket: t?.id || null, recorded };
  },
  ...
  // the engineer. The customer reply is HELD (handled in executePlay) — we don't tell them it's
  // fixed when it isn't. This is the "engineering queue": escalated items live in #gymnasticbodies-development.
...
export async function executePlay({ member_email, response, actions, case_id }) {
  ...
  const steps = [];
  // Escalations HOLD the reply — never tell a customer it's fixed when it's queued for engineering.
  const holdReply = (actions || []).some((a) => a.type === 'escalate');
  if (response && response.trim() && !holdReply) {
  ...
  } else if (holdReply && response && response.trim()) {
    steps.push({ step: 'reply', ok: true, note: 'held — escalated to engineering (draft kept, not sent)' });
  }
