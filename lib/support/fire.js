...
export async function fireOne(f) {
  ...
  if (f.channel && f.thread_ts) {
    ...
    const banner = `${result.ok ? '✅ *Sent*' : '⚠️ *Sent with issues*'}${closed ? ' · case closed' : ''}${escalated ? ' · escalated to engineering' : ''}`;
    if (f.play_ts) await slack('chat.update', { channel: f.channel, ts: f.play_ts, text: 'Sent', blocks: doneBlocks(banner, f, playOf(f)) });
