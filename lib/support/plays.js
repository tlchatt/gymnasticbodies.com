...
export function actionLabel(a) {
  ...
  if (a.type === 'reopen') return `set case${p.case_id ? ` #${p.case_id}` : ''} to open`;
  if (a.type === 'legacy_offer') return 'create legacy offer (legacy15 · $15/mo — makes the /offer link work)';
  if (a.type === 'escalate') return `escalate to engineering${p.note ? ` — ${p.note}` : (a.summary ? ` — ${a.summary}` : '')}`;
  if (a.type === 'reassign') return `reassign${p.assigned_to || p.team ? ` to ${p.assigned_to || p.team}` : ''}`;
