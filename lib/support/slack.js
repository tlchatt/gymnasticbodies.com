...
function contentBlocks(fire, play) {
  ...
  const lines = [];
  if (fire.response && String(fire.response).trim()) lines.push(escalates ? '⛔ Reply held — escalated to engineering (not sent)' : 'Send reply');
  ...
  const ctx = [
    `*${play.issue_class || fire.issue_class || 'other'}*`,
