...
/**
 * Clean plan/product name for display. The legacy `productName` embeds an OLD price,
 * e.g. "GymnasticBodies Unlimited – $29.99 USD / Month" or "GymFit TV - $239.88 USD / Year".
 * That label contradicts the real Price row (which is what the member actually pays), and
 * panicked members on the $15 legacy rate into thinking the promo failed. Strip the embedded
 * price so the Plan label is just the plan name; the separate Price row is the source of truth.
 * ~3,902 members carried a mismatched label. Returns the input untouched if it has no price.
 */
export function cleanPlanName(name) {
  if (!name) return name;
  const cleaned = String(name).replace(/\s*[–—-]?\s*\$[\d.,]+.*$/i, '').trim();
  return cleaned || 'GymFit Membership';
}
