...
// Returns all support-related messages received after `sinceDate` (JS Date).
//
// Query rationale (see claudePlans/mail-reconciliation-2026-07-24.md — the old
// `list:`-only query under-ingested 281 messages over 540 days):
// - `list:support@`            → Google Group deliveries (contact forms, group-tagged replies)
// - `to:`/`deliveredto:support@` → direct mail to support@ missing the List-ID header
// - `to:admin@ category:personal` → human replies sent straight to admin@ (excludes the
//   automated DMARC/alert flood, which is category:updates)
// - `in:anywhere` + includeSpamTrash → DMARC-rewritten group replies are frequently
//   spam-filed; without this the sync can never see them
export async function fetchDigestsSince(sinceDate) {
  const gmail = getGmailClient();
  const afterEpoch = Math.floor(sinceDate.getTime() / 1000);
  const q =
    'in:anywhere (' +
    'list:support@gymnasticbodies.com' +
    ' OR to:support@gymnasticbodies.com' +
    ' OR deliveredto:support@gymnasticbodies.com' +
    ' OR (to:admin@gymnasticbodies.com category:personal)' +
    `) after:${afterEpoch}`;
  const messages = [];
  let pageToken;
  do {
