...
export async function getUserWithEmail(email) {
    ...
    // Exact match first — keeps the handful of accounts where both casings exist
    // as separate users deterministic. Case-insensitive fallback second: the Apr
    // 2026 bulk import carried ~2k mixed-case emails members can never type back
    // (login lowercases input), locking their own accounts away from every
    // email-keyed lookup on this path.
    let returnUser = await db.select().from(user).where(eq(user.email, email));
    if (returnUser.length === 0) {
        returnUser = await db.select().from(user).where(sql`lower(${user.email}) = ${String(email).toLowerCase()}`);
    }
