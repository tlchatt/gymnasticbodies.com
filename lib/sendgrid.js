...
// Deliver an in-app "Contact Support" message into the support@ Gmail inbox as a real email, so the
// team has it in Gmail alongside emailed tickets and can reply from there. The in-app route already
// records the message + case + Slack fire directly; THIS is an additional copy for the inbox. It
// carries an X-GB-Source header so the Gmail sync recognizes it and does NOT re-ingest it (which
// would create a duplicate case + Slack thread). Best-effort — returns true/false, never throws.
export async function sendInAppSupportToInbox({ memberEmail, memberName, subject, body, caseId, adminBase }) {
  try {
    sgMail.setApiKey(process.env.SENDGRID_API_KEY)
    const esc = (s) => String(s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    const caseLink = caseId && adminBase
      ? `<p><strong>Case:</strong> <a href="${adminBase}/admin/cases/${caseId}">#${caseId}</a></p>` : ''
    const html = `<h2>${esc(subject) || 'Contact Support'}</h2>
      <p><strong>From:</strong> ${esc(memberName || memberEmail)} &lt;${esc(memberEmail)}&gt;</p>
      ${caseLink}
      <hr/>
      <p>${esc(body).replace(/\n/g, '<br/>')}</p>
      <hr/>
      <p style="color:#888;font-size:12px">Sent from the in-app Contact Support form on my.gymnasticbodies.com.</p>`
    await sgMail.send({
      to: 'support@gymnasticbodies.com',
      from: { email: process.env.SENDGRID_FROM, name: memberName ? `${memberName} (in-app support)` : 'In-app support' },
      replyTo: memberEmail,
      subject: subject || 'Contact Support',
      headers: { 'X-GB-Source': 'inapp-support' },
      content: [{ type: 'text/html', value: html }],
    })
    return true
  } catch (error) {
    console.error('sendInAppSupportToInbox failed:', error?.message)
    return false
  }
}
